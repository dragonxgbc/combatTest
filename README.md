...
| Key | Action |
|---|---|
| Mouse Button 1 (hold) | M1 combo (up to 4 hits) |
| F (hold) | Block |
| Q | Dash |
| 1 | Black Flash |
...
All tunable values live in `src/shared/CombatConfig.luau`. Both server and client read from it. Sections:
...
> **Note:** `BLACK_FLASH_COOLDOWN` is commented out in config. The fallback `or 12` in `CooldownManager` and `ClientCooldowns` is what's active.
...
Special move (press 1). Validates the same gates as M1 plus its own per-player `lastUsed` timestamp against `CooldownManager.get("blackFlash")`. Applies full-duration endlag immediately, broadcasts to all clients (triggers animation + arm aura), then after `windupTime` delay performs the hitbox scan.

| Target state | Damage | Knockback | Posture | Hitstun/Stagger |
|---|---|---|---|---|
| Unblocked | Full | Yes | +30 | Yes |
| Blocked (front) | None | No | +30 | Yes |
| Blocked (back-hit bypass) | Full | Yes | +30 | Yes |
...
Manages all movement and action restrictions.
...
A background coroutine per affected character (20 Hz) applies WalkSpeed/JumpPower overrides and restores them when all locks expire.
...
Guard pressure bar (0 → `POSTURE_MAX`).
...
A background loop runs passive linear decay every `POSTURE_DECAY_TICK` (0.1s), decaying from current value to 0 over `POSTURE_DECAY_TIME` (22s).
...
These scripts are designed to be placed **inside Parts in the workspace**. Each Part's `Touched` event drives the behavior.
...
| Part attribute | Behavior |
|---|---|
| `InvulnGive = true` | Grants `Invulnerable` on touch |
| `InvulnGive` absent or false | Removes `Invulnerable` on touch |
...
Press-1 special move client handler.
...
HUD slots for Dash [Q] and Black Flash [1], anchored bottom-center of screen.
